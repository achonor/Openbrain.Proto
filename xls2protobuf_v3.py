...
#proto文件输出路径
PROTO_FILE_OUT_PATH = "xls2proto/"
#bin文件输出路径
BIN_FILE_OUT_PATH = "xls2bin/"
#txt文件输出路径
TXT_FILE_OUT_PATH = "xls2txt/"
...
class DataParser:
    ...
    def Parse(self) :
        ...
        self._WriteReadableData2File(str(item_array).encode())
    ...
    def _ParseField(self, max_repeated_num, repeated_num, item) :
        ...
        if field_rule == "optional" :
            ...
            if second_row.isdigit() or second_row.find(".") != -1 :
                ...
                #field_value_str = (self._sheet.cell_value(self._row, self._col)).strip()
...
def getAllFile(path):
    ...
    for x in os.listdir(path):
        ...
        extend_name = os.path.splitext(x)[-1]
        if (os.path.isfile(file_path) and ('.xlsx' == extend_name or '.xls' == extend_name)):
...
if __name__ == '__main__' :
    """入口"""
    #sys.argv.append("E:/Openbrain.Proto/xls")
    if len(sys.argv) < 2 :
        print ("Need one parameter")
        sys.exit(-1)

    all_file = getAllFile(sys.argv[1])
    ...
    for file_path in  all_file:
        ...
        for sheet_name in xls_file.sheet_names():
            #生成proto文件
            try :
                tool = SheetInterpreter(xls_file, sheet_name)
                tool.Interpreter()
            except BaseException as e :
                print ("Interpreter Failed!!!")
                print (e)
                sys.exit(-3)
            print("Interpreter Success!!!" + sheet_name)
            #生成bin文件
            try:
                parser = DataParser(xls_file, sheet_name)
                parser.Parse()
            except BaseException as e:
                print("Parse Failed!!!")
                print(e)
                sys.exit(-4)
            print("Parse Success!!!" + sheet_name)
